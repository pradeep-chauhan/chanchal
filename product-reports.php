            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="ibox float-e-margins">
                        ...
                        <div class="ibox-content jeans">
                            <form method="" class="form-horizontal" action="#">
                                <div class="form-group">
                                    <label class="col-sm-2 control-label">Select Date</label>
                                    <div class="input-daterange input-group col-sm-5" id="datepicker">
                                        <input type="text" class="input-sm form-control" name="start" id="start" placeholder="select date">
                                        <span class="input-group-addon">to</span>
                                        <input type="text" class="input-sm form-control" name="end" id="end" placeholder="select date">
                                    </div>
                                ...
                                <div id="no_print5" class="form-group">
                                    <div class="col-sm-4 col-sm-offset-2">
                                        <button class="btn btn-primary" name="btn_search_company_stock" id="btn_search_product_stock" type="button">Search</button>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-2 ">
                                        <input type="text" name="" id="grand_total_value" class="form-control" readonly/>
                                    </div>
                                </div>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('#btn_search_product_stock').click(function(){
            ...
            $.ajax({
                ...
                type:'GET',
                dataType:'JSON',
                data:{search_product_details:1,Product_category_id:$product_category_id,Start:$start,End:$end},
                success:function(data){
                    ...
                    if(data.Jeans.length>0) {
                        ...
                        for(var i=0;i<data.Jeans.length;i++) {
                            ...
                            $template.find('td').eq(0).html(data.Jeans[i].company_name);
                    ...
                    if(data.Shirt.length>0) {
                        ...
                        for(var i=0;i<data.Shirt.length;i++) {
                            ...
                            $template.removeClass('hidden');
                            $template.addClass('append');
                            $template.find('td').eq(0).html(data.Shirt[i].company_name);
                    ...
                    if(data.Capri.length>0) {
                        ...
                        for(var i=0;i<data.Capri.length;i++) {
                            ...
                            $template.removeClass('hidden');
                            $template.addClass('append');
                            $template.find('td').eq(0).html(data.Capri[i].company_name);
                            ...
                            $template.find('td').eq(8).html(data.Capri[i].fourty);
                    ...
                    if(data.under_garments.length>0) {
                        ...
                        for(var i=0;i<data.under_garments.length;i++) {
                            ...
                            $template.removeClass('hidden');
                            $template.addClass('append');
                            $template.find('td').eq(0).html(data.under_garments[i].company_name);
                            ...
                            $template.find('td').eq(8).html(data.under_garments[i].one_hundre_five);
                    ...
                    if(data.others.length>0) {
                        ...
                        for(var i=0;i<data.others.length;i++) {
                            ...
                            $template.removeClass('hidden');
                            $template.addClass('append');
                            $template.find('td').eq(0).html(data.others[i].company_name);
                            $template.find('td').eq(1).html(data.others[i].product_name);
                            $template.find('td').eq(2).html(data.others[i].article_no);
                            $template.find('td').eq(3).html(data.others[i].quantity);
        ...
        $('#btn_print_click').click(function(){
            ...
            var $start=$('#start').val();
            var $end=$('#end').val();
            window.open('view-product-reports.php?product_category_id='+$product_category_id+'&start='+$start+'&end='+$end,'_blank');
        });
    });
</script>
